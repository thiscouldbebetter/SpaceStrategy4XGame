...
class TalkSession
{
	...
	log: string[];
	scopeCurrent: TalkScope;
	_variablesByName: Map<string,any>;

	optionSelected: TalkNode;

	constructor(defn: TalkDefn, factions: Faction[])
	{
		this.defn = defn;
		...
		this._variablesByName = new Map<string,any>();
	...
	static buildExample(factionActing: Faction, factionReceiving: Faction)
	{
	...
	static fromStrings(factions: Faction[], talkNodesAsStrings: string[])
	{
		var talkNodes = [];
