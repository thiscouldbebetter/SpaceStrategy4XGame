...
class Constraint_Cursor {
    ...
    constrain(uwpe) {
        ...
        if (cursor.hasXYPositionBeenSpecified == false) {
            ...
            var collisionPos = new CollisionExtended().rayAndPlane(rayFromCameraToMousePos, planeToRestrictTo).pos;
        ...
        else // if (cursor.hasXYPositionBeenSpecified)
         {
            ...
            cursorPos.z = 0;
            var planeToRestrictTo = new Plane(planeNormal, cursorPos.dotProduct(planeNormal));
            var collisionPos = new CollisionExtended().rayAndPlane(rayFromCameraToMousePos, planeToRestrictTo).pos;
            if (collisionPos != null) {
