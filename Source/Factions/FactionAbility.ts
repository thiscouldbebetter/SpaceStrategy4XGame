...
class FactionAbility
{
	...
	private _perform: (uwpe: UniverseWorldPlaceEntities) => void;
	description: string;
...
class FactionAbility_Instances
{
	...
	constructor()
	{
		var fa = (name: string, rtc: number, perform: (uwpe: UniverseWorldPlaceEntities)=>void, descr: string) =>
			new FactionAbility(name, rtc, perform, descr);
	...
	charm(uwpe: UniverseWorldPlaceEntities): void
	{
		var world = uwpe.world as WorldExtended;
	...
	clairvoyize(uwpe: UniverseWorldPlaceEntities): void
	{
		var world = uwpe.world as WorldExtended;
		var factionSelf = world.factionCurrent();
	...
	cram(uwpe: UniverseWorldPlaceEntities): void
	{
		var world = uwpe.world as WorldExtended;
		var factionSelf = world.factionCurrent();
	...
	energize(uwpe: UniverseWorldPlaceEntities): void
	{
		var world = uwpe.world as WorldExtended;
		var factionSelf = world.factionCurrent();
	...
	enervate(uwpe: UniverseWorldPlaceEntities): void
	{
		var world = uwpe.world as WorldExtended;
		var factionSelf = world.factionCurrent();
	...
	farsee(uwpe: UniverseWorldPlaceEntities): void
	{
		var world = uwpe.world as WorldExtended;
	...
	hermitize(uwpe: UniverseWorldPlaceEntities): void
	{
		var world = uwpe.world as WorldExtended;
		var factionSelf = world.factionCurrent();
		var starsystems = factionSelf.starsystems(world);
	...
	inspire(uwpe: UniverseWorldPlaceEntities): void
	{
		var world = uwpe.world as WorldExtended;
		var factionSelf = world.factionCurrent();
	...
	intractablize(uwpe: UniverseWorldPlaceEntities): void
	{
		var world = uwpe.world as WorldExtended;
		var factionSelf = world.factionCurrent();
		var planets = factionSelf.planets;
	...
	larcenize(uwpe: UniverseWorldPlaceEntities): void
	{
		var world = uwpe.world as WorldExtended;
		var factionSelf = world.factionCurrent();
		...
		var factionsKnowingByTechnologyName = new Map<string, Faction[]>();
		var technologiesKnownByAtLeastTwoOthers = new Array<Technology>();

		technologiesAll.forEach
		(
			tech =>
			{
				...
				for (var f = 0; f < factionsOther.length; f++)
				{
					...
					{
						var factionsKnowing =
							factionsKnowingByTechnologyName.get(techName);

						if (factionsKnowing == null)
						{
							factionsKnowing = new Array<Faction>();
							factionsKnowingByTechnologyName.set
							(
								techName, factionsKnowing
							);
						}
						else
						{
		...
		(
			notificationText, null // todo - jumpTo
		);

		factionSelf.notificationSession.notificationAdd(notification);
	}

	manipulate(uwpe: UniverseWorldPlaceEntities): void
	{
		alert("todo - faction ability - manipulate");
	}

	nurture(uwpe: UniverseWorldPlaceEntities): void
	{
		var world = uwpe.world as WorldExtended;
		var factionSelf = world.factionCurrent();
		var planets = factionSelf.planets;
	...
	prolificate(uwpe: UniverseWorldPlaceEntities): void
	{
		var world = uwpe.world as WorldExtended;
		var factionSelf = world.factionCurrent();
		var planets = factionSelf.planets;
	...
	regenerate(uwpe: UniverseWorldPlaceEntities): void
	{
		var world = uwpe.world as WorldExtended;
		var factionSelf = world.factionCurrent();
		var ships = factionSelf.ships;
	...
	telepathicize(uwpe: UniverseWorldPlaceEntities): void
	{
		var world = uwpe.world as WorldExtended;
		var factionSelf = world.factionCurrent();
		var factionSelfDiplomacy = factionSelf.diplomacy;
		...
		for (var i = 0; i < factionsOther.length; i++)
		{
			...
			var relationship =
				factionSelfDiplomacy.relationshipByFaction(factionOther);
			relationship.stateSetToPeace();
		}
	}

	xenophobicize(uwpe: UniverseWorldPlaceEntities): void
	{
		...
		for (var i = 0; i < starsystems.length; i++)
		{
			...
			var shipsInStarsystem = starsystem.ships;
			var shipsInStarsystemBelongingToOthers =
				shipsInStarsystem.filter(x => x.factionable().faction() != factionSelf);

			for (var s = 0; s < shipsInStarsystemBelongingToOthers.length; s++)
			{
				var shipToExpel = shipsInStarsystemBelongingToOthers[s];
				var linkPortalToExpelThrough =
