...
class BuildableDefnsBasic {
    constructor(mapCellSizeInPixels) {
        ...
        description);
        var facilitySurfaceUsable = (name, visual, industryToBuildAmount, effectPerRound, description) => new BuildableDefn(name, false, // isItem
        canBeBuiltOnSurfaceUsable, mapCellSizeInPixels, visual, industryToBuildAmount, effectPerRound, null, // effectsAvailableToUse
        null, // categories
        null, // entityProperties
        null, // entityModifyOnBuild
        description);
        var facilitySurfaceAnywhere = (name, visual, industryToBuildAmount, description) => new BuildableDefn(name, false, // isItem
        ...
        null, // entityModifyOnBuild
        description);
        var shipComponent = (name, visual, industryToBuildAmount, deviceDefn, description) => new BuildableDefn(name, true, // isItem
        ...
        this.OrbitalCloak = facilityOrbital("Orbital Cloak", visualBuild("C", colors.Gray), 120, "Hides orbital structures from sensors.");
        this.OrbitalDocks = facilityOrbital("Orbital Docks", visualBuild("D", colors.Gray), 120, "Allows ships to be repaired and refitted.");
        this.OrbitalShield = facilityOrbital("Orbital Shield", visualBuild("S", colors.Red), 60, "Protects orbital structures and excludes others' ships from orbit.");
        this.OrbitalShieldAdvanced = facilityOrbital("Orbital Shield, Advanced", visualBuild("S", colors.Blue), 120, "A stronger version of the orbital shield.");
        this.OrbitalWeaponBasic = facilityOrbital("Orbital Weapon, Basic", visualBuild("W", colors.Gray), 60, "Allows planet to attack ships in the starystem.");
        this.OrbitalWeaponIntermediate = facilityOrbital("Orbital Weapon, Intermediate", visualBuild("W", colors.Red), 120, "A more powerful orbital weapon.");
        this.OrbitalWeaponAdvanced = facilityOrbital("Orbital Weapon, Advanced", visualBuild("W", colors.Green), 240, "An even more powerful orbital weapon.");
        this.PlanetwideFocusDiplomacy = planetwideFocus("Planetwide Diplomacy Focus", visualBuild("Focus", colors.Orange), "Planet's industry is directed toward improving diplomatic relations.");
        this.PlanetwideFocusResearch = planetwideFocus("Planetwide Research Focus", visualBuild("Focus", colors.Blue), "Planet's industry is redirected to research.");
        ...
        this.SurfaceCloak = facilitySurfaceUsable("Surface Cloak", visualBuild("Cloak", colors.Gray), 120, effectNone, "Conceals surface structures from sensors.");
        ...
        this.SurfaceLaboratoryMultiplier = facilitySurfaceUsable("Laboratory Multiplier", visualBuild("LM", colors.Pink), 120, effectNone, "Doubles the effect of all research-producing structures.");
        this.SurfaceOutpost = facilitySurfaceUsable("Outpost", visualBuild("Outpost", colors.Pink), 120, effectTodo, "Adds 1 to max population.");
        ...
        this.SurfaceTransportTubes = facilitySurfaceAnywhere("Transport Tubes", visualBuild("Transp", colors.Red), 15, "Allows traversal of otherwise unusable terrain.");
        this._All =
            [
                this.OrbitalCloak,
                this.OrbitalDocks,
