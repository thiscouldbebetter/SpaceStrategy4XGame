...
class StarCluster extends PlaceBase
{
	...
	): StarCluster
	{
		...
		var nodesNotYetLinked =
			this.generateRandom_1(universe, nodeDefns, numberOfNodes, radiusMax);

		var [nodesLinked, links] =
			this.generateRandom_2(universe, radiusMax, nodesNotYetLinked);
	...
	static generateRandom_2
	(
		universe: Universe,
		radiusMax: number,
		nodesNotYetLinked: StarClusterNode[]
	): [ StarClusterNode[], StarClusterLink[] ]
	{
		var numberOfNodes = nodesNotYetLinked.length;

		var nodePositions = nodesNotYetLinked.map
		(
			x => Locatable.of(x).loc.pos
		);
		var boundsActual = BoxAxisAligned.create().containPoints(nodePositions);
		var boundsDesired = BoxAxisAligned.fromCenterAndSize
		(
	...
	placeForEntityLocatable(entityLocatable: Entity): any
	{
		...
		var placeTypeColonName =
			Locatable.of(entityLocatable).loc.placeName();
	...
	drawForCamera(universe: Universe, camera: Camera): void
	{
		...
		for (var i = 0; i < entitiesDrawableToSort.length; i++)
		{
			...
			(
				drawPos.overwriteWith(Locatable.of(entityDrawableToSort).loc.pos)
			);
	...
	): ControlBase
	{
		...
		var labelRound = ControlLabel.fromPosSizeTextFontUncentered
		(
		...
		var textRound = ControlLabel.fromPosSizeTextFontUncentered
		(
			Coords.fromXY
			(
