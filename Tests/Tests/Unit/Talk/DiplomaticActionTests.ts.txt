...
{
	name: string;
	effect: (u: Universe, factionActing: Faction, factionReceiving: Faction) => void;
	...
	static _instances: DiplomaticAction_Instances;
	static Instances()
	{
...
{
	Alliance: DiplomaticAction;
	Peace: DiplomaticAction;
	War: DiplomaticAction;

	_All: DiplomaticAction[];
	...
	constructor()
	{
		...
		(
			"Declare War on",
			(universe: Universe, factionActing: Faction, factionReceiving: Faction) =>
			{
				var message;

				var relationshipStates = DiplomaticRelationship.States();
				var stateExisting =
					factionActing.relationshipByFactionName(factionReceiving.name).state;
				...
				{
					var relationship =
						factionActing.relationshipByFactionName(factionReceiving.name);
		...
		(
			...
			(universe: Universe, factionActing: Faction, factionReceiving: Faction) =>
			{
				...
				else // if (stateExisting == DiplomaticRelationship.States().War)
				{
					...
					(
						world, factionReceiving.enemies(world)
					);

					var strengthOfSelfAndAllies = DiplomaticRelationship.strengthOfFactions
					(
						world, factionReceiving.selfAndAllies(world)
					);
					...
					if (strengthOfAlliesMinusEnemies <= 0)
					{
						...
						var relationship =
							factionActing.relationshipsByFactionName.get(factionReceiving.name);
		...
		(
			...
			(universe: Universe, factionActing: Faction, factionReceiving: Faction) =>
			{
				...
				var stateExisting =
					factionActing.relationshipsByFactionName.get(factionReceiving.name).state;
				var relationshipStates = DiplomaticRelationship.States();
				...
				else
				{
					...
					var enemiesOfEitherFaction = new Array<Faction>();
					...
					for (var f = 0; f < factions.length; f++)
					{
						...
						var intersection = enemiesOfFactionThis.filter
						(
							x => alliesOfFactionOther.indexOf(x) >= 0
						);
					...
					if (doAlliesAndEnemiesOfFactionsClash == false)
					{
						...
						(
							world, factionsDeclaringWarOnReceiving
						);
